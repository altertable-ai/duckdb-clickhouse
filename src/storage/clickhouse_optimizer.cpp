...
//! A LIMIT may only move into ClickHouse when every table filter of the scan is either evaluated by ClickHouse
//! itself, or is not required for correctness in the first place.
//!
...
//! exists.
//!
//! Every filter DuckDB pushes into a *required* (non-optional) position is guaranteed to either translate to a
//! real SQL fragment or make ClickhouseFilterPushdown::TransformFilter throw -- it never silently drops a
//! required predicate (see its header comment). The only filters that can translate to "" without throwing are
//! TableFilterType::OPTIONAL_FILTER / DYNAMIC_FILTER / BLOOM_FILTER (and a CONJUNCTION_OR with such a branch):
//! DuckDB's own definition of OPTIONAL_FILTER is "executing filter is not required for query correctness", and
//! DYNAMIC_FILTER/BLOOM_FILTER are runtime pruning hints generated by *other* operators (a join, or -- the
//! concrete case this scanner hits -- DuckDB's own TopN optimizer, which unconditionally attaches an
//! OptionalFilter(DynamicFilter) to the scan for every ORDER BY ... LIMIT it turns into a LogicalTopN, to help
//! its own heap prune row groups). That hint's correctness is owed entirely to the LogicalTopN/LogicalLimit
//! node we are about to remove and replace with an exact ClickHouse-side ORDER BY/LIMIT; once removed, the
//! now-orphaned hint is never evaluated by anyone (dropped, not enforced) but was never required for
//! correctness to begin with, so removing it changes nothing. Blocking pushdown on an empty (non-throwing)
//! translation would therefore make ORDER BY ... LIMIT pushdown dead code for the exact tables and column
//! types it is meant to help: see task-8-report.md for the EXPLAIN evidence ("optional: Dynamic Filter (n)")
//! that a plain `ORDER BY n DESC LIMIT 3` already carries one of these hints today.
