...
// Derived from ClickHouse's own src/Common/ErrorCodes.cpp (via clickhouse-cpp 2.6.2's vendored
// clickhouse/error_codes.h, itself based on a Feb 2024 snapshot of upstream). Regenerate by extracting
// "NAME = NUMBER," lines from that header if clickhouse-cpp is ever upgraded and new codes are needed.
static const unordered_map<int32_t, const char *> CLICKHOUSE_ERROR_CODE_NAMES = {
	{0, "OK"},
...
const char *ClickhouseErrorCodeName(int32_t code) {
	auto it = CLICKHOUSE_ERROR_CODE_NAMES.find(code);
	if (it == CLICKHOUSE_ERROR_CODE_NAMES.end()) {
		return "";
	}
	return it->second;
}
