...
- **Semantics:** translated DML follows ClickHouse semantics where it differs from DuckDB (NaN comparisons, UUID ordering, integer division by zero). This is documented in the README; SELECT pushdown remains exact-only.
  ...
  > - The count and the statement run with `transform_null_in = 0` whatever the ATTACH's `settings=` say (query settings override the connection's). It is the count that needs it: on 25.8 the lightweight DELETE and the mutation ignore `transform_null_in`, from the query's settings and from the server's default profile alike.
