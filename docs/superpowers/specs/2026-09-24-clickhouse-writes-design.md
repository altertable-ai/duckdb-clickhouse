...
- **Value conversion** (`clickhouse_writer`):
  ...
  - Types the read path converts on the server (IPv4/IPv6, (U)Int256, Decimal P > 38, JSON/Object/Variant/Dynamic, geo types) are written as String and converted by the server: `INSERT INTO db.t (cols) SELECT <conversion>(c), … FROM input('c String, …')`. As built: into a Nullable column the conversion is `if(isNull(c), NULL, CAST(assumeNotNull(c) AS T))`, because `CAST(c AS Nullable(T))` turns text that does not parse into NULL; such text fails the INSERT. On 25.8 the CAST also wraps (U)Int256 text out of range around (2^256 into UInt256 is 0, `accurateCast` wraps the same way) and truncates Decimal(P > 38) digits beyond the scale (`'1.009'` into Decimal(76, 2) is 1.00; digits beyond the precision fail). So for those types it runs inside `tupleElement(tuple(throwIf(NOT <valid>, '<message>'), CAST(x AS T)), 2)`: (U)Int256 text must match `^[+-]?[0-9]+$` and print back (`toString(CAST(x AS T))`) as the text without its `+` and leading zeros (`'+007'` is 7, `'-0'` is 0 in Int256 and fails in UInt256, which refuses a `-`); Decimal text must be plain decimal notation (`^[+-]?([0-9]+\.?|[0-9]*\.[0-9]+)$`: `'1.'`, `'.5'`, a sign and leading zeros are fine, `'1e3'` is not) with no non-zero digit beyond the scale (`'1.500'` is fine). A value under a typed JSON path (`JSON(a Int32)`) is converted by ClickHouse's own rules (`{"a":1.9}` stores 1); this is documented, not checked. **Plan-time spike:** check that clickhouse-cpp's `BeginInsert` works with `input()`. If it does not, INSERT into those column types is rejected with an error pointing to `clickhouse_execute`, and the README says so.
...
> - SET/DROP DEFAULT that has to materialize the column first requires `ch_mutations_sync = 2` on a `Replicated*` or `Shared*` (ClickHouse Cloud) engine: with 1 the other replicas could apply the default change before the mutation.
...
- **Semantics:** translated DML follows ClickHouse semantics where it differs from DuckDB (NaN comparisons, UUID ordering, integer division by zero). This is documented in the README; SELECT pushdown remains exact-only.
  ...
  > - SET of a column ClickHouse cannot store NULL in (anything but `Nullable`, `LowCardinality(Nullable)`, `Variant` and `Dynamic`): a NULL constant raises DuckDB's `ConstraintException` (`NOT NULL constraint failed: t.c`) while planning, even when no row matches (nothing is sent; a WHERE DuckDB folds to false still reports 0); any other value that may be NULL (not a constant, nor a column that cannot hold NULL) is counted first, `SELECT countIf(isNull(<value>)), … FROM db.t WHERE p` with `apply_deleted_mask = 0`, and a count above 0 raises the same error. On 25.8 the mutation fails on the first NULL (error 349) and stays stuck, masked (deleted, not purged) rows included; `Array`, `Tuple`, `Map` and `JSON` values are never NULL in ClickHouse (a NULL fails when the statement is analysed). Their *elements* can be, and a NULL element into a non-Nullable element type fails the mutation the same way (error 349, stuck), while `isNull(<array>)` is always 0. So a SET of an Array/Tuple/Map column (any level, Map values included) whose element types cannot hold NULL is decided while planning: a constant with such a NULL element raises the `ConstraintException`; a plain reference to another column is refused when that column's ClickHouse element types can hold NULL where the target's cannot (`Array(Nullable(Int32))` into `Array(Int32)`); any other value is refused (DuckDB's own types are always nullable, so nothing tells it cannot hold a NULL element). Both refusals are shape errors pointing to `clickhouse_execute()`. The mutation also runs with the server profile's `short_circuit_function_evaluation`: a profile with `'disable'` would evaluate `CAST(n AS Int32)` on rows the WHERE leaves out.
